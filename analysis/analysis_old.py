...
def filter_query(end, snc):
    def f():
        snc.execute(
            "SELECT * FROM intron WHERE chrom='chr1' AND start>=0 AND start<= ? AND end<=? AND samples_count>=100",
            (end, end))
        snc.fetchall()
...
def filtercount_query(end, snc):
    def f():
        snc.execute(
            "SELECT count(*) FROM intron WHERE chrom='chr1' AND start>=0 AND start<= ? AND end<=? AND samples_count>=100",
            (end, end))
        snc.fetchall()
...
def simple_query(end, snc):
    def f():
        snc.execute("SELECT * FROM intron WHERE chrom='chr1' AND start>=0 AND start<= ? AND end<=?",
                    (end, end))
        snc.fetchall()
...
def simplecount_query(end, snc):
    def f():
        snc.execute("SELECT count(*) FROM intron WHERE chrom='chr1' AND start>=0 AND start<= ? AND end<=?",
                    (end, end))
        snc.fetchall()
...
def fetch_size(end, snc):
    snc.execute("SELECT count(*) FROM intron WHERE chrom='chr1' AND start>=0 AND start<= ? AND end<=?",(end, end))
    count_simple = snc.fetchone()[0]
    snc.execute("SELECT count(*) FROM intron WHERE chrom='chr1' AND start>=0 AND start<= ? AND end<=? AND samples_count>=100",(end, end))

    return count_simple, snc.fetchone()[0]

sconn = sqlite3.connect('./junctions.sqlite')
snc = sconn.cursor()
...
with open('old_times.csv', 'w+') as f:
    w = csv.writer(f)
    w.writerows(times)